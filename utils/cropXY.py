...
def cropXY(input_=None,N=None):
    ...
    if N > max(Nx,Ny): # invalid/ missing input N => square the stack
        if Nx != Ny:
            ...
            #out=input_(arange(ox + 1,ox + N),arange(oy + 1,oy + N),arange()) # beware matlab indexing
            #print(str(input_.shape))
            #print("ox:{}, oy:{}, N:{}".format(ox,oy,N))
            out=input_[ox:ox+N-1,oy:oy+N-1,:]
        else:
            out=copy(input_)
        ...
        oy=int(np.floor((Ny - N) / 2))
        #out=input_(arange(ox + 1,ox + N),arange(oy + 1,oy + N),arange())
