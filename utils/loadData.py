...
def loadData(path=None,*args,**kwargs):
    ...
    if path is None:
        ...
        print("Files retrieved from Filenames[0]: {}\n shape: {}".format(filenames[0], im_.shape))
        #im_ = np.array(skio.imread(filenames[0], plugin="tifffile"))
    else: 
    ...
    image_batch = np.empty((z, dims[0], dims[1], dims[2]), dtype=np.uint16) #dtype=np.uint8
    for idx, file in enumerate(filenames):
        try:
            ...
            print('File {:s} failed'.format(file))
    
    image_batch = image_batch.squeeze()      
    print("Finished reading...")
    print("Stack size: {}".format(image_batch.shape))
